...
public class ServerUserInfoWindow extends JInternalFrame {
	...
	public ServerUserInfoWindow(String windowName, int x, int y, int width, int height, boolean resizable,
			boolean closable) {
		super();
	...
	public void initialize(String windowName, int x, int y, int width, int height, boolean resizable,
		...
		try {
				...
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {
						try {
							 ...
							 * (((String)listModel.get(i)).indexOf(txt_UserName.
							 * getText()) > -1) { jJList.setSelectedIndex(i);
							...
							if (player != null) {
								...
								JOptionPane.showMessageDialog(null, txt_UserName.getText() + "님은 현재 월드상에 존재하지 않습니다.",
					...
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							...
							if (pc != null) {
								...
								iptable.banIp(pc.getNetConnection().getIp()); // BAN
																				// 리스트에
																				// IP를
																				// 추가한다.
					...
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							...
							if (pc != null) {
								...
								pc.sendPackets(new S_ServerMessage(286, String.valueOf(10))); // \f3게임에
																								// 적합하지
																								// 않는
																								// 행동이기
																								// (위해)때문에,
				...
				public void actionPerformed(ActionEvent e) {
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							...
							jServerPresentWindow.setLocation(
									(eva.jJFrame.getContentPane().getSize().width / 2)
											- (jServerPresentWindow.getContentPane().getSize().width / 2),
									(eva.jJFrame.getContentPane().getSize().height / 2)
											- (jServerPresentWindow.getContentPane().getSize().height / 2));
					...
					if (eva.isServerStarted) {
						if (!txt_UserName.getText().equalsIgnoreCase("")) {
							if (!txt_UserName.getText().equalsIgnoreCase("")) {
								...
								jServerPolyWindow.setLocation(
										(eva.jJFrame.getContentPane().getSize().width / 2)
												- (jServerPolyWindow.getContentPane().getSize().width / 2),
										(eva.jJFrame.getContentPane().getSize().height / 2)
												- (jServerPolyWindow.getContentPane().getSize().height / 2));
			...
			main.addGroup(layout.createSequentialGroup()
					.addComponent(jJTabbedPane1, GroupLayout.PREFERRED_SIZE, 150, GroupLayout.PREFERRED_SIZE)
			...
			vertical_grp.addGap(3).addContainerGap().addGroup(layout.createBaselineGroup(false, false)
					.addComponent(txt_UserName).addComponent(btn_Search).addComponent(chk_Infomation));

			vertical_grp.addGap(3).addContainerGap().addGroup(
					layout.createBaselineGroup(true, true).addComponent(jJTabbedPane1).addComponent(jJTabbedPane2));

			vertical_grp.addGap(3).addContainerGap()
					.addGroup(layout.createBaselineGroup(false, false).addComponent(lbl_Helper).addComponent(btn_Ban)
							.addComponent(btn_NoChat).addComponent(btn_Present).addComponent(btn_Poly)
							.addComponent(btn_AllPresent).addComponent(btn_AllPoly));

			vertical_grp.addGap(3).addContainerGap()
					.addGroup(layout.createBaselineGroup(false, false).addComponent(lbl_UserCount));
		...
		try {
			...
			if (chk_Infomation.isSelected()) {
				...
				if (player != null) {
					...
					String[] lbls = { "Class", "Level", "HP", "MP", "Weight", "Lawful", "Str(Total/Base)",
							"Dex(Total/Base)", "Con(Total/Base)", "Int(Total/Base)", "Wis(Total/Base)",
							"Cha(Total/Base)", "Hpr", "Mpr", "[일반]공격성공", "[일반]추가타격", "[일반]활명중치", "[일반]활타격치", "[갑옷]공격성공",
							"[갑옷]추가타격", "[갑옷]활명중치", "[갑옷]활타격치", "[인형]활명중치", "[인형]활타격치", "[일반]데미지리덕", "[인형]데미지리덕" };
					...
					for (int i = 0; i < lbls.length; i++) {
						...
						case 2:
							...
							break;
						case 3:
							charInfo[1] = String.valueOf(player.getAbility().getTotalStr()) + " / "
									+ player.getAbility().getBaseStr();
							charInfo[3] = String.valueOf(player.getAbility().getTotalDex()) + " / "
									+ player.getAbility().getBaseDex();
							break;
						case 4:
							charInfo[1] = String.valueOf(player.getAbility().getTotalCon()) + " / "
									+ player.getAbility().getBaseCon();
							charInfo[3] = String.valueOf(player.getAbility().getTotalInt()) + " / "
									+ player.getAbility().getBaseInt();
							break;
						case 5:
							charInfo[1] = String.valueOf(player.getAbility().getTotalWis()) + " / "
									+ player.getAbility().getBaseWis();
							charInfo[3] = String.valueOf(player.getAbility().getTotalCha()) + " / "
									+ player.getAbility().getBaseCha();
							break;
						case 6:
	 ...
	 * //txt_UserName.setText((String)listModel.elementAt(jJList.
	 * getSelectedIndex()));
	 * 
	 * if (chk_Infomation.isSelected()) { L1PcInstance player =
	 * L1World.getInstance().getPlayer(txt_UserName.getText());
	 * DefaultTableModel tModel1 = (DefaultTableModel)jJTable1.getModel();
	 * DefaultTableModel tModel2 = (DefaultTableModel)jJTable2.getModel();
	 * DefaultTableModel tModel3 = (DefaultTableModel)jJTable3.getModel();
	 * DefaultTableModel tModel4 = (DefaultTableModel)jJTable4.getModel();
	 ...
	 * String.valueOf(player.getAbility().getTotalStr()) + " / " +
	 * player.getAbility().getBaseStr(); charInfo[3] =
	 * String.valueOf(player.getAbility().getTotalDex()) + " / " +
	 * player.getAbility().getBaseDex(); break; case 4: charInfo[1] =
	 * String.valueOf(player.getAbility().getTotalCon()) + " / " +
	 * player.getAbility().getBaseCon(); charInfo[3] =
	 * String.valueOf(player.getAbility().getTotalInt()) + " / " +
	 * player.getAbility().getBaseInt(); break; case 5: charInfo[1] =
	 * String.valueOf(player.getAbility().getTotalWis()) + " / " +
	 * player.getAbility().getBaseWis(); charInfo[3] =
	 * String.valueOf(player.getAbility().getTotalCha()) + " / " +
	 ...
	 * ItemTable.getInstance().createItem(rs1.getInt("item_id"));
	 ...
	 * "SELECT login, ip, host, phone FROM accounts WHERE ip = "); sb.append(
	 * "(SELECT ip FROM accounts WHERE login = "); sb.append(
	 * "(SELECT account_name FROM characters WHERE char_name = ?))");
	 * 
	 * con = L1DatabaseFactory.getInstance().getConnection(); pstm =
