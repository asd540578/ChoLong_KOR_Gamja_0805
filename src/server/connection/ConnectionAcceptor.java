...
public class ConnectionAcceptor extends Thread /* implements Monitorable */{
	...
	private void createClientThread(Socket connection) {
		/*
		 * LineageClient client = null; try { client = new
		 * LineageClient(connection);
	...
	// }
	/*
	 * @Override public void notifyMonitors() { String s = null;
	 * 
	 * switch(state) { case STARTUP: s = "[LoginServer] Started.."; break; case
	 * WAITING: //s = "[LoginServer : 메모리] " + SystemUtil.getUsedMemoryMB() +
	 * "MB 사용"; s = "[LoginServer : 접속] 대기 中..."; break; case CONNECTION: s =
	 * "[LoginServer] Connected Client IP: " + clientIp; break; case BAN: s =
	 * "[LoginServer] Baned Client IP: " + clientIp + " (count : 7)"; break;
	 * default: if(s == null) return; }
	 * 
	 * monitor.update(s); }
	 * 
	 * @Override public void registerMonitor(Monitor m) { monitor = m; }
	 * 
	 * @Override public void removeMonitor(Monitor m) { if(monitor.equals(m))
	 * monitor = null; }
	 */
}
