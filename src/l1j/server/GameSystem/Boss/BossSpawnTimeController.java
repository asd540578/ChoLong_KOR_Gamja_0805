...
public class BossSpawnTimeController implements TimeListener {
		...
		if (Config.접속채팅모니터() > 0) {
			for (L1PcInstance gm : Config.toArray접속채팅모니터()) {
				...
				gm.sendPackets(new S_SystemMessage("[" + b.getName() + "] "
						+ newH + "시 " + newM + "분 / 주기 : " + perih + "시 "
						+ perim + "분"));
			}
		}
		System.out.println("Name : " + b.getName());
		System.out.println("다음스폰 " + newH + "시 " + newM + "분 / 주기 : " + perih
				+ "시 " + perim + "분");
	...
	public void onMinuteChanged(BaseTime time) {
		...
		int sH, sM;
		for (L1BossCycle b : L1BossCycle.getBossCycleList()) {
			if (H == b.getKillHour() && M == b.getKillMinute()) { //이벤트로그오류
		//		eva.EventLogAppend(" ["+time.get(Calendar.HOUR)+"시"+time.get(Calendar.MINUTE)+"분] "+b.getName() + " [알림] 보스 타임이 종료되었습니다" );
				...
				if (Config.접속채팅모니터() > 0) {
					for (L1PcInstance gm : Config.toArray접속채팅모니터()) {
						...
						gm.sendPackets(new S_SystemMessage("킬보스" + b.getName()));
					}
				}
			}
			...
			if (sH == H && sM == M) {
//이벤트로그오류
			//	eva.EventLogAppend(" ["+time.get(Calendar.HOUR)+"시"+time.get(Calendar.MINUTE)+"분] "+b.getName() +" [알림] 보스 타임이 시작되었습니다");
				...
				if (Config.접속채팅모니터() > 0) {
					for (L1PcInstance gm : Config.toArray접속채팅모니터()) {
						...
						gm.sendPackets(new S_SystemMessage("스폰보스" + b.getName()));
					}
				}
			}
		}
