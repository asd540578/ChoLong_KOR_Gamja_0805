...
public class BadukiChatController implements Runnable {
		...
		public void run() {
			...
			try {
				S_ChatPacket s_chatpacket = new S_ChatPacket(
						bot,
						"\\\\fTf2★할배게임★온라인NO.1유저수 저커팅 실카규칙!원탁바둑이!!방수60개이상www.ccoo20.com",
						Opcodes.S_SAY, 0);
	...
	public void OnOff(L1PcInstance gm, boolean ck) {
		if (ck) {
			...
			on = true;
			gm.sendPackets(new S_SystemMessage("바둑이 채팅 로봇이 가동되었습니다."), true);
			return;
		} else {
			if (L1World.getInstance().findObject(list.get(0).getId()) == null) {
			...
			on = false;
			gm.sendPackets(new S_SystemMessage("바둑이 채팅 로봇이 종료되었습니다."), true);
			return;
		}
	}
