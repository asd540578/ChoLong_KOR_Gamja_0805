...
public class S_RangeSkill extends ServerBasePacket {
	...
	public S_RangeSkill(L1Character cha, L1Character[] target, int spellgfx,
		...
		buildPacket(cha, cha.getX(), cha.getY(), target, spellgfx, actionId,
				type);
	}

	public S_RangeSkill(L1Character cha, int x, int y, L1Character[] target,
			int spellgfx, int actionId, int type) {
		buildPacket(cha, x, y, target, spellgfx, actionId, type);
	}

	private void buildPacket(L1Character cha, int x, int y,
			L1Character[] target, int spellgfx, int actionId, int type) {
		try {
			...
			for (int i = 0; i < target.length; i++) {
				...
				writeH(0x20); // 0:데미지 모션 있음 0이외:없음
			}
			writeH(0x00);
		} catch (Exception e) {
		}
