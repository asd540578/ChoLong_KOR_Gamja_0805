...
public class L1UltimateBattle {
	...
	private void sendRoundMessage(int curRound) {
		...
		case 5:
			if (curRound == 1) {
				...
				sendMessage("\\fS콜롯세움 관리인: 제 3 군 투입!"); // 스타트
			} else if (curRound == 4) {
	...
	private void spawnSupplies(int curRound) {
		...
		case 2:
			if (curRound == 1) {
				spawnGroundItem(L1ItemId.ADENA, 3000, 40);
				spawnGroundItem(L1ItemId.POTION_OF_CURE_POISON, 3, 20);
				// spawnGroundItem(L1ItemId.POTION_OF_GREATER_HEALING, 5, 20);
				spawnGroundItem(40011, 5, 20); // 주홍이
				spawnGroundItem(40012, 3, 20); // 말갱이
				spawnGroundItem(40079, 1, 20); // 귀환
				sendMessage("\\fS제 1 군의 투입이 완료되었습니다.");
				sendMessage("\\fS약 1분 후에 제 2 군의 투입이 시작됩니다.");
			} else if (curRound == 2) {
				spawnGroundItem(L1ItemId.ADENA, 15000, 30);
				spawnGroundItem(40011, 20, 20); // 주홍이
				spawnGroundItem(40012, 10, 20); // 말갱이
				spawnGroundItem(L1ItemId.POTION_OF_CURE_POISON, 7, 20);
				spawnGroundItem(40079, 1, 20); // 귀환
				spawnGroundItem(40012, 1, 20); // 빈줌(4레벨)
				spawnGroundItem(40317, 1, 20); // 숫돌
				sendMessage("\\fS제 2 군의 투입이 완료되었습니다.");
				sendMessage("\\fS약 2분 후에 제 3 군의 투입이 시작됩니다.");
			} else if (curRound == 3) {
				spawnGroundItem(40012, 1, 20); // 빈줌(4레벨)
				spawnGroundItem(40079, 1, 20); // 귀환
				spawnGroundItem(40011, 40, 20); // 주홍이
				spawnGroundItem(40012, 20, 20); // 말갱이
				spawnGroundItem(L1ItemId.POTION_OF_CURE_POISON, 7, 20);
				spawnGroundItem(L1ItemId.ADENA, 30000, 30);
				spawnGroundItem(40317, 1, 20); // 숫돌
				sendMessage("\\fS제 3 군까지의 투입이 완료되었습니다.");
				sendMessage("\\fS약 1분 후에 최종전이 시작됩니다.");
			}
			break;
	...
	class UbThread implements Runnable {
		private void countDown() throws InterruptedException {
			...
			sendMessage("\\fS콜롯세움 관리인: 이제 곧 몬스터가 등장할 것입니다."); // 10초전
			Thread.sleep(5000);
			removeRetiredMembers();
			sendMessage("\\fS콜롯세움 관리인: 10초뒤에 경기를 시작 합니다."); // 10초전

			Thread.sleep(5000);
			sendMessage("\\fS콜롯세움 관리인: 5 !!"); // 5초전
			removeRetiredMembers();

			Thread.sleep(1000);
			sendMessage("\\fS콜롯세움 관리인: 4 !!"); // 4초전
			removeRetiredMembers();

			Thread.sleep(1000);
			sendMessage("\\fS콜롯세움 관리인: 3 !!"); // 3초전
			removeRetiredMembers();

			Thread.sleep(1000);
			sendMessage("\\fS콜롯세움 관리인: 2 !!"); // 2초전
			removeRetiredMembers();

			Thread.sleep(1000);
			sendMessage("\\fS콜롯세움 관리인: 1 !!"); // 1초전
			removeRetiredMembers();
		...
		public void run() {
			try {
					...
					if (_ubId == 1 || _ubId == 2) {
						if (round == 3) { // 3군 시작
							for (L1PcInstance pc : getMembersArray()) {
								pc.getInventory().storeItem(41402, 3);
								pc.sendPackets(new S_ServerMessage(403,
										"용자의 표식 (3)"), true);
								// pc.sendPackets(new
								// S_SystemMessage("용자의 표식  3개를 얻었습니다."));
							}
						} else if (round == 4) {
							for (L1PcInstance pc : getMembersArray()) {
								pc.getInventory().storeItem(41402, 5);
								pc.sendPackets(new S_ServerMessage(403,
										"용자의 표식 (5)"), true);
								// pc.sendPackets(new
								// S_SystemMessage("용자의 표식  5개를 얻었습니다."));
							}
						}
					}
				...
				for (L1PcInstance pc : getMembersArray()) {
					...
					pc.sendPackets(new S_ServerMessage(403, "용자의 표식 (10)"),
							true);
					// pc.sendPackets(new
					// S_SystemMessage("용자의 표식  10개를 얻었습니다."));
					pc.getInventory().storeItem(435010, 1);
					pc.sendPackets(new S_ServerMessage(403, "무한대전 승리의 조각"),
							true);
					// pc.sendPackets(new
					// S_SystemMessage("무한대전 승리의 조각을 얻었습니다."));
	...
	public boolean canPcEnter(L1PcInstance pc) {
		...
		if (!((pc.isCrown() && _enterRoyal)
				|| ((pc.isKnight() || pc.isWarrior()) && _enterKnight)
