...
public class L1WorldTraps {
	...
	private void initialize() {
		...
		try {
			...
			while (rs.next()) {
				...
				// System.out.println(base.getX()+" > "+base.getY()+" > "+base.getMapId());
				_allBases.add(base);
			}
		...
		if (trap.isRespawn()) {
			synchronized (this) {
				GeneralThreadPool.getInstance().schedule(
						timer = new TrapSpawnTimer(trap), trap.getSpan());
	...
	public void addTrap(L1TrapInstance trap) {
		_allTraps.add(trap);
	}

	public void addBase(L1TrapInstance trap) {
		_allBases.add(trap);
	}

	private class TrapSpawnTimer implements Runnable {// extends TimerTask {
