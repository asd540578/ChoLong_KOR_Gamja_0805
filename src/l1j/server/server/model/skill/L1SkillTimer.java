...
class L1SkillStop {
	public static void stopSkill(L1Character cha, int skillId) {
		...
		case 타이탄라이징: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem(타이탄라이징, 1));	
			}
		}
		break;
		case 어쌔신: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem(어쌔신, 1));	
			}
		}
		break;
		case 임팩트: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem(임팩트, 1));
				pc.임팩트 = 0;
			}
		}
		break;
		case 그레이스아바타: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem( 그레이스아바타, 1));
				pc.getResistance().addhorror(- pc.그레이스아바타);
				pc.getResistance().addStun(- pc.그레이스아바타);
				pc.getResistance().addHold(- pc.그레이스아바타);
				pc.그레이스아바타 = 0;
			}
		}
		break;
		case 소울배리어: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem( 소울배리어, 1));	
			}
		}
		break;
		case 데스힐: {
		...
		case BRAVE_AVATAR2:
			...
			cha.getAbility().addAddedDex((byte) -1);
			cha.getAbility().addAddedInt((byte) -1);
			cha.getResistance().addMr(-9);
			cha.getResistance().addStun(-2);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SPMR(pc), true);
				pc.sendPackets(new S_PacketBox(S_PacketBox.NONE_TIME_ICON, 0,
		...
		case SHINING_AURA:
			...
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillIconAura(114, 0), true);
			}
			break;
		case BRAVE_AURA:
			// cha.addDmgup(-5);
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillIconAura(116, 0), true);
			}
			break;
		case RESIST_MAGIC:
			cha.getResistance().addMr(-10);
		...
		case EARTH_SKIN:
			...
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillIconShield(6, 0), true);
			}
			break;
		case ERASE_MAGIC:
			if (cha instanceof L1PcInstance) {
				...
				if (_cha instanceof L1PcInstance) {
					...
					if (!attacker.isGmInvis()) {
						Broadcaster.broadcastPacket(attacker, new S_PinkName(
								attacker.getId(), _remainingTime), true);
					}
