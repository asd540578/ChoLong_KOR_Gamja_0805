...
public class L1PcInstance extends L1Character {
	...
	private static String[] _diementArray = { "벌써죽냐" , "ㅡㅡ", "다이ㅋㅋ", "ㅋㅋㅋㅋㅋㅋ","","개족밥ㅋ",""};
	...
	public void 삼단가속() {// 13283
		 ...
		 * 디케이포션 상태 pc.sendPackets(new S_ServerMessage(698), true); return; }
		 */
		...
		int time = 600 * 1000;
		int stime = (int) (((time / 1000) / 1)); //진주 시간
	...
	public void Stat_Reset_All() {
		...
		sendPackets(new S_NewCreateItem(0x01e3, 1, getAbility().getTotalStr(),
				getAbility().getTotalCon(), "힘", getType(), this));
		...
		sendPackets(new S_NewCreateItem(0x01e3, 1, getAbility().getTotalCon(),
				getAbility().getTotalStr(), "콘", getType(), this));
		...
		public void run() {
			...
			for (int i = 0; i < allBuffComaSkill.length; i++) {
				if (allBuffComaSkill[i] == 7895) {
					...
					getSkillEffectTimerSet().setSkillEffect(L1SkillId.흑사의기운,
							1800 * 1000);
				} else {
	...
	public void 사망패널티(boolean login) {
		if (login || _oldzone != CharPosUtil.getZoneType(this)) {
			if (CharPosUtil.getZoneType(this) == 1) {
				sendPackets(new S_NewCreateItem(S_NewCreateItem.사망패널티, true),
						true); // 사망패널티?
			} else {
				sendPackets(new S_NewCreateItem(S_NewCreateItem.사망패널티, false),
						true); // 사망패널티?
			}
			...
			if (LastINDay != null) {
				...
				if (LastINDay.getDate() != nowday.getDate()) {
					...
					if (lasthours < Config.D_Reset_Time
							&& hours >= Config.D_Reset_Time) {// 같은날 9시이전에
																// 들어간거체크
						return maxtime;
					}
				}
	...
	public long TamTime() {
		...
		try {
			...
			pstm = con
					.prepareStatement("SELECT `TamEndTime` FROM `characters` WHERE account_name = ? ORDER BY `TamEndTime` ASC"); // 케릭터
																																	// 테이블에서
																																	// 군주만
																																	// 골라와서
		...
		if (getAC().getAc() < -250) {
			if (Config.버그채팅모니터() > 0) {
				for (L1PcInstance gm : Config.toArray버그채팅모니터()) {
					...
					gm.sendPackets(new S_SystemMessage("방어구 버그 의심 " + getName()
							+ " AC : " + ac.getAc() + " 팅김 처리함"), true);
	...
	private void statcheck(int i, int type) {
		...
		if (Config.버그채팅모니터() > 0) {
			for (L1PcInstance gm : Config.toArray버그채팅모니터()) {
				...
				gm.sendPackets(new S_SystemMessage("스탯 버그 의심 " + getName()
						+ " " + type_text + i + " 팅김 처리함"), true);
	 ...
	 * if(item.getCount() > GameServer.getInstance().아덴최저값){
	 * if(Config.버그채팅모니터()>0){ for(L1PcInstance gm : Config.toArray버그채팅모니터()){
	 ...
	 * GameServer.getInstance().아덴최저값){ if(Config.버그채팅모니터()>0){ for(L1PcInstance
	 * gm : Config.toArray버그채팅모니터()){ if(gm.getNetConnection()==null){
	...
	public void onChangeExp() {
		...
		if (gap > 0) {
			levelUp(gap);
			if (getLevel() >= 60) {
				getSkillEffectTimerSet().setSkillEffect(L1SkillId.레벨업보너스,
						10800000);
		...
		if ((isGm() && isGmInvis()) || (isSGm() && isGmInvis())) {
			...
			if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.TRUE_TARGET)) {
				if (tt_clanid == pc.getClanid()
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.이미지스폰, getId(),
							12299, true));
				}
			}
		}
					...
					if (visible instanceof L1PcInstance) {
						...
						if (pc.getSkillEffectTimerSet().hasSkillEffect(
							...
							if (pc.tt_clanid == getClanid()
								...
								sendPackets(new S_PacketBox(S_PacketBox.이미지스폰,
										pc.getId(), 12299, true));
							}
						}
			...
			if (getCurrentHp() > 0 && !isDead()) {
				...
				if (attack.calcHit()) {
					if (attacker.getWeapon() != null) {
						...
						if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.타이탄라이징)){ //타이탄라이징
							rock = 5;
							if(getLevel() > 80)
								rock += getLevel()-80;
							if(rock > 10)
	...
	public void onAction(L1PcInstance attacker) {
		...
		try {
			...
			if (getCurrentHp() > 0 && !isDead()) {
				...
				//	System.out.println("타이탄락구간 = " + rock);			
					for (L1DollInstance doll : getDollList()) {
						if (doll.getDollType() == L1DollInstance.DOLLTYPE_뱀파이어)
							rock+=5;		
					}
				//	System.out.println("인형락구간 = " + rock);		
					if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.타이탄라이징)){ //타이탄라이징
						int r = 5;
						if(getLevel() > 80)
							r += getLevel()-80;
				...
				//	System.out.println("라이징락구간 = " + rock);						
					if (attacker.getWeapon() != null) {
						if (isTaitanM
								&& attacker.getWeapon().getItem().getType() == 17) {
							int hpRatio = 100;
	...
	public synchronized void receiveDamage(L1Character attacker, double damage,
		...
		try {
				...
				if (damage > 0) {
					...
					if (attacker instanceof L1PcInstance) {
						...
						if (isMagicDamage
							...
							S_DoActionGFX 데미지 = new S_DoActionGFX(getId(),
				...
				if (newHp <= 0) {
					if (isGm() || isSGm()) {
						...
						if (getInventory().checkItem(41159, 50)) { //인벤에 깃50개 있나확인
				               ...
				               } else if(getInventory(). checkItem(40308, 500000)){//50만 체크
						...
						if (CharPosUtil.getZoneType(L1PcInstance.this) == 0) // 노멀존
							if (getHuntCount() > 0) {
								...
								if (CharPosUtil.getZoneType(this) == 0 && CharPosUtil.getZoneType(attacker) == 0) {
									L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("["+attacker.getName()+ "]님이 [" +this.getName()+"] 를[을] 죽였습니다." ));  
								}
							}
						}
						...
						/** 2011.05.08 고정수 배틀존 */
		 {
				...
				System.out
						.println("[L1PcInstance] 경고 : 죽은 대상 현재HP 0 이상. 데미지 입음. 캐릭터: "
								+ getName() + " 공격자: " + attacker != null ? attacker
								.getName() : "없음");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void death(L1Character lastAttacker) {
		...
		if (lastAttacker instanceof L1PcInstance) {
			...
			if (CharPosUtil.getZoneType(this) == 0
				...
				// L1World.getInstance().broadcastServerMessage(lastAttacker.getName()+" 이 "
				// +this.getName()+ "의 14번 척추를 좌우로 비틀어 버립니다.");

				// L1World.getInstance().broadcastServerMessage(lastAttacker.getName()+" 이 "
				// +this.getName()+ "을 죽였습니다.",false);
				...
				/*
				 * if (this instanceof L1RobotInstance ||
				 * getInventory().checkItem(41159, 50)) { //인벤에 깃50개 있나확인
				 * //L1World.getInstance().broadcastServerMessage( //월드메세지 송신 //
				 * getName() + "는 멘붕상태로 " +lastAttacker.getName() +
				 * "한테 깃털 50개 상납");//PK승리자 메세지표시
				 * L1World.getInstance().broadcastServerMessage( //월드메세지 송신
				 * getName() + "은 " +lastAttacker.getName() +
				 * "한테 깃털 50개를 주었습니다.");//PK승리자 메세지표시
				 * lastAttacker.getInventory().storeItem(41159, 50); //승리자 깃50개
				 * 받아오기 if(!(this instanceof L1RobotInstance)){
				 * getInventory().consumeItem(41159, 50); //패배자 깃 50개 삭제
				 * S_SystemMessage sm = new
				 * S_SystemMessage("전투에서 패배하여 깃털(50)을 잃었습니다."); sendPackets(sm);
				 * sm.clear(); sm = null; } }else{//깃부족시 물약뺏어오기
				 ...
				 * + "는 깃털조차 없어 " +lastAttacker.getName() + "한테 아덴 상납");//PK승리자
				 * 메세지표시 lastAttacker.getInventory().storeItem(40308, 500000);
				 * //패패자 아이템 잃는다 L1World.getInstance().broadcastServerMessage(
				 * getName() + "는 깃털이 없어 " +lastAttacker.getName() +
				 * "한테 아덴을 주었습니다.");//PK승리자 메세지표시
				 * lastAttacker.getInventory().storeItem(40308, 500000); //패패자
				 * 아이템 잃는다 S_SystemMessage sm = new
				 * S_SystemMessage("깃털이 부족하여 아덴 50만을 잃었습니다."); sendPackets(sm);
				 * sm.clear(); sm = null; }else{
				 * L1World.getInstance().broadcastServerMessage(getName() +
				 * "은 깃털 및 아덴이 없어 잃지 않았습니다.");//PK승리자 메세지표시 } } if
				 * (getHuntCount() > 0){
				 ...
				 * + " 이 " + getName() + "의 14번 척추를 비틀어 현상금 획득");
	...
	int stunskillid[] = { SHOCK_STUN, MOB_SHOCKSTUN_30, MOB_RANGESTUN_19,
			...
			FREEZING_BREATH, L1SkillId.데스페라도, BONE_BREAK, PHANTASM,
	...
	private class Death implements Runnable {
		...
		public void run() {
			...
			//		System.out.println("333");
				...
				if (getInventory().checkEquipped(222501) && (getMapId() == 1700 || getMapId() == 1703)) { // 고대의 가호
				...
				// 65레벨 이하일때 10렙높은 유저에게 죽임을 당하면 패널티 없음.
				//if (lastAttacker instanceof L1PcInstance && getLevel() < 65 && (lastAttacker.getLevel() - getLevel()) >= 10) {
				//	sendPackets(new S_SystemMessage("\\fW조우의 가호로 인해 경험치는 떨구지않습니다."));
				//	return;
				//} else if (getLevel() > 51) {
				//	deathPenalty();
				//}
				...
				if (!isFirstBlood()) {
					 sendPackets(new S_SystemMessage("첫 죽음을 경험 하셨습니다."));
					setFirstBlood(true);
				}
				...
				if (lastAttacker instanceof L1PcInstance) {
					...
					t.sendPackets(new S_PacketBox(S_PacketBox.배틀샷,
				...
				if (player != null) {
					...
					//		sendPackets(new S_SystemMessage("여기 지나갑니다 376 에러"));
					//		setPinkName(false);
	...
	public void resExpTo구호() {
		...
		if (level > Config.MAXLEVEL) {
			S_SystemMessage sm = new S_SystemMessage(
					"레벨 제한으로 인해  더이상 경험치를 획득할 수 없습니다.");
			sendPackets(sm, true);
			return;
		}
...
//잊섬
	...
	private void levelUp(int gap) {
		...
		if (getLevel() >= Config.AUTO_REMOVECLAN && BloodName.equalsIgnoreCase("신규버프")) {
			try {
				...
				for (int i = 0; i < clanMember.length; i++) {
					clanMember[i].sendPackets(new S_ServerMessage(ServerMessage.LEAVE_CLAN, player_name, clan_name)); // \f1%0이 %1혈맹을 탈퇴했습니다.
				}
		...
		for (int i = 0; i < gap; i++) {
			...
			int randomMp = 0;
			if (old++ <= 51) {
				randomHp = CalcStat.레벨업피(getType(), getBaseMaxHp(),
						getAbility().getTotalCon());
				randomMp = CalcStat.레벨업엠피(getType(), getBaseMaxMp(),
		...
		if (getLevel() >= 82 && lv52_step != L1Quest.QUEST_END) {
			item = getInventory(). storeItem(888820, 1); //영웅 지원 상자
		...
		if (getLevel() >= 99) { // 지정 레벨
			if ((getMapId() >= 25 && getMapId() <= 28)
					|| (getMapId() >= 2221 && getMapId() <= 2232)) {// 수련던전
				L1Teleport.teleport(this, 33080, 33390, (short) 4, 5, true); // WB
			}
		}
	...
	class 텔스레드 implements Runnable {
		...
		/*
		 * 데스나이트의 정신이 스며듭니다.(남은시간: 30분)18648 데스나이트의 의지가 느껴집니다.(남은시간: 20분)18649
		 * 검이 더 거세게 불타 오릅니다.(남은시간: 10분)18650 검의 힘에 눌리기 시작합니다.(남은시간: 5분)18651 시간이
		 * 얼마 안남았음을 느낍니다.(남은시간: 3분)18652 의식이 희미해집니다.(남은시간: 1분)18653
		 */
		@Override
		public void run() {
			try {
	...
	private void levelDown(int gap) {
		...
		for (int i = 0; i > gap; i--) {
			...
			int randomMp = 0;
			if (old-- <= 51) {
				randomHp = CalcStat.레벨업피(getType(), 0, getAbility()
						.getTotalCon());
				randomMp = CalcStat.레벨업엠피(getType(), 0, getAbility()
						.getTotalWis());
			} else {
				randomHp = CalcStat.레벨업피(getType(), 0, getAbility()
						.getTotalCon());
		...
		if (!isGm() && Config.LEVEL_DOWN_RANGE != 0) {
			if (getHighLevel() - getLevel() >= Config.LEVEL_DOWN_RANGE) {
				...
				if (getNetConnection() != null)
					getNetConnection().close();
				_log.info(String.format("[%s]: 렙따 범위를 넘었기 때문에 강제 절단 했습니다.",
						getName()));
			}
		}
	 ...
	 * = 0; int newBaseStatDmgup = CalcStat.근거리대미지(getAbility().getTotalStr());
	 * int newBaseStatBowDmgup = CalcStat.원거리대미지(getAbility().getTotalDex()); if
	...
	public void cancelAbsoluteBarrier() { // 아브소르트바리아의 해제
		...
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_안전모드)) {
			getSkillEffectTimerSet()
					.killSkillEffectTimer(L1SkillId.STATUS_안전모드);
		}
	}
	...
	public void calAinHasad(int i) {
		 ...
		 * i; if(calc <= 9999){//아인하사드 최초소멸 체크 aincheck = true; } if (calc >=
...
sendPackets(new S_SPMR(this));
	...
	public void removePet(L1NpcInstance npc) {
		...
		for (L1NpcInstance petV : getPetList()) {
			if (petV.getId() == npc.getId()) {
				...
				// sendPackets(new S_SystemMessage("펫갯수.."+petV.length));
	...
	protected int calcSleepTime() {
		...
		if (interval == 0) {
			...
			//	System.out.println(this.getName() + "의 spr오류 확인 요망 변신 : "
			...
			if (weaponType == 20
				...
				if (getGfxId().getTempCharGfx() == 7968
					...
					Broadcaster.broadcastPacket(this, ua);
						...
						|| this.getGfxId().getTempCharGfx() == 8900) { // 헬바인
					S_UseArrowSkill ua = new S_UseArrowSkill(this, 0, 8904,
							target.getX(), target.getY(), true);
					sendPackets(ua);
					Broadcaster.broadcastPacket(this, ua);
						...
						|| this.getGfxId().getTempCharGfx() == 8913) { // 질리언
					S_UseArrowSkill ua = new S_UseArrowSkill(this, 0, 8916,
							target.getX(), target.getY(), true);
					sendPackets(ua);
					Broadcaster.broadcastPacket(this, ua);
