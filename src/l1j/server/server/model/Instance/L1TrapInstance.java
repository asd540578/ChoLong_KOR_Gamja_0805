...
public class L1TrapInstance extends L1Object {
	...
	public void setRespawn(boolean ck) {
		respawn = ck;
	}
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		...
		if (perceivedFrom.getSkillEffectTimerSet().hasSkillEffect(
			...
			S_Trap st = new S_Trap(this, _nameForView);
			perceivedFrom.sendPackets(st, true);
			_knownPlayers.add(perceivedFrom);
		} else {
			if (_trap.getId() >= 101 && _trap.getId() <= 111) {
				perceivedFrom.getNearObjects().addKnownObject(this);
				S_Trap st = new S_Trap(this, _trap.getGfxId(), "");
				perceivedFrom.sendPackets(st, true);
				_knownPlayers.add(perceivedFrom);
			}
		}
