...
public class L1PetInstance extends L1NpcInstance {
	...
	public boolean noTarget() {
		if (_currentPetStatus == 3) {
			...
			if (getLocation().getTileLineDistance(_petMaster.getLocation()) > 2) {
				int dir = moveDirection(_petMaster.getMapId(),
	...
	public L1PetInstance(L1Npc template, L1PcInstance master, L1Pet l1pet) {
		...
		setExpPercent((int) ExpTable.getExpPercentage(l1pet.get_level(),
				l1pet.get_exp()));
	...
	public synchronized void death(L1Character lastAttacker) {
		if (!isDead()) {
			...
			if (_petMaster == null || _petMaster.getNetConnection() == null
					|| !_petMaster.getLocation().isInScreen(getLocation()))
				dropItem();
			else
				collect();
		}
	...
	public void collect() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (_petMaster.getInventory().checkAddItem(item, item.getCount()) == L1Inventory.OK) {
				...
				S_ServerMessage sm = new S_ServerMessage(143, getName(),
						item.getLogName());
				_petMaster.sendPackets(sm, true);
			} else {
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		...
		S_PetPack pp = new S_PetPack(this, perceivedFrom);
		perceivedFrom.sendPackets(pp, true);
		if (perceivedFrom == getMaster()) {
			setCurrentHp(getCurrentHp());
		}

		if (isDead()) {
			S_DoActionGFX oa = new S_DoActionGFX(getId(),
					ActionCodes.ACTION_Die);
			perceivedFrom.sendPackets(oa, true);
		}
	}
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (_petMaster.equals(player)) {
			S_PetMenuPacket pm = new S_PetMenuPacket(this, getExpPercent());
			player.sendPackets(pm, true);
			L1Pet l1pet = PetTable.getInstance().getTemplate(_itemObjId);
			if (l1pet != null) {
				l1pet.set_exp(getExp());
				l1pet.set_level(getLevel());
	...
	public void onFinalAction(L1PcInstance player, String action) {
		...
		if (status == 6) {
			...
			L1PetType type = null;
			for (Object petObject : _petMaster.getPetList()) {
				if (petObject instanceof L1PetInstance) {
					...
					if (_petMaster != null
						...
						if (id != 0) {
							Broadcaster
									.broadcastPacket(this, new S_NpcChatPacket(
											pet, "$" + id, 0), true);
						}
					}
				}
	...
	private int actionType(String action) {
		int status = 0;
		if (getMapId() == 5125 || getMapId() == 5131 || getMapId() == 5132
				|| getMapId() == 5133 || getMapId() == 5134) {
			_petMaster.sendPackets(
					new S_SystemMessage("펫 매치 중에는 사용 할 수 없습니다."), true);
			return 0;
		}
	...
	private void setPetWeapon(L1ItemInstance weapon) {
		...
		addDmgup(petItem.getDamageModifier());
		addHitup(petItem.getHitModifier());
