...
public class HpRegeneration extends TimerTask {
	...
	public void regenHp() {
		...
		// /----여기서부터
		if (L1CastleLocation.checkInAllWarArea(_pc.getX(), _pc.getY(),
				_pc.getMapId())) {
			for (Object petObject : _pc.getPetList()) {
				if (petObject instanceof L1PetInstance) { // 펫
					L1PetInstance pet = (L1PetInstance) petObject;
					pet.dropItem();
					// _pc.getPetList().remove(pet.getId());
					_pc.removePet(pet);
					pet.deleteMe();
				}
					...
					// _pc.getPetList().remove(sunm.getId());
					_pc.removePet(sunm);
					sunm.deleteMe();
				}

				else if (petObject instanceof L1DollInstance) { // 마법인형
					L1DollInstance sunm = (L1DollInstance) petObject;
					// _pc.getPetList().remove(sunm.getId());
					_pc.removePet(sunm);
					sunm.deleteMe();
				}
