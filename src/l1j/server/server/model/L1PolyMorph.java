...
public class L1PolyMorph {
	...
	public static void handleCommands(L1PcInstance pc, String s, int time) {
		...
		// System.out.println("변신 string : "+s);
	...
	public static void doPoly(L1Character cha, int polyId, int timeSecs,
		...
		if (cha instanceof L1PcInstance) {
			...
			if (cha.getMapId() != 5143 && !isMatchCause(polyId, cause)) {
				pc.sendPackets(new S_ServerMessage(181)); // \f1 그러한 monster에게는
															// 변신할 수 없습니다.
				return;
			}
			...
			try {
				...
				pc.getSkillEffectTimerSet().setSkillEffect(
						L1SkillId.SHAPE_CHANGE, timeSecs * 1000);
				if (pc.getGfxId().getTempCharGfx() != polyId) {
					...
					if (!pc.isGmInvis()) {
						Broadcaster.broadcastPacket(pc,
								new S_ChangeShape(pc.getId(), polyId));
					}
					...
					if (weapon != null) {
						...
						pc.sendPackets(new S_PacketBox(
								S_PacketBox.WEAPON_RANGE, pc, weapon, true),
								true);
					} else {
						pc.sendPackets(new S_PacketBox(
								S_PacketBox.WEAPON_RANGE, pc, weapon, false),
								true);
					}
	...
	public static void undoPoly(L1Character cha) {
		if (cha instanceof L1PcInstance) {
			...
			/*if(pc.getGfxId().getTempCharGfx() >= 13715 && pc.getGfxId().getTempCharGfx() <= 13745){
				...
				int time = pc.getSkillEffectTimerSet().getSkillEffectTimeSec(MORPH_BY_ITEMMAGIC);
				doPoly(pc, polyId, time, L1PolyMorph.MORPH_BY_ITEMMAGIC);
			} else*/ 
