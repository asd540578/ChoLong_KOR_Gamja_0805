...
public class WhiteThunderWand extends L1ItemInstance {
	...
	public void clickItem(L1Character cha, ClientBasePacket packet) {
		if (cha instanceof L1PcInstance) {
			...
			if (target != null) {
				...
				pc.sendPackets(new S_UseAttackSkill(pc, 0, 11736, spellsc_x,
						spellsc_y, 17), true);
				Broadcaster.broadcastPacket(pc, new S_UseAttackSkill(pc, 0,
						11736, spellsc_x, spellsc_y, 17), true);
			}
			pc.플레이어상태 = pc.공격_상태;
	...
	private void doWandAction(L1PcInstance user, L1Object target) {
		...
		int dmg = ((_random.nextInt(11) + 1) + user.getAbility().getTotalStr() / 4) * 2;
		dmg = Math.max(22, dmg);
		...
		if (target instanceof L1PcInstance) {
			...
			if (CharPosUtil.getZoneType(pc) == 1
				...
				user.sendPackets(new S_UseAttackSkill(user, pc.getId(), 11736,
						pc.getX(), pc.getY(), 17, 0), true);
				Broadcaster.broadcastPacket(user,
						new S_UseAttackSkill(user, pc.getId(), 11736,
								pc.getX(), pc.getY(), 17, 0), true);
			...
			user.sendPackets(
					new S_UseAttackSkill(user, pc.getId(), 11736, pc.getX(), pc
							.getY(), 17), true);
			...
			user.sendPackets(
					new S_UseAttackSkill(user, mob.getId(), 11736, mob.getX(),
							mob.getY(), 17), true);
			Broadcaster.broadcastPacket(user,
					new S_UseAttackSkill(user, mob.getId(), 11736, mob.getX(),
							mob.getY(), 17), true);
			...
			if (npc instanceof L1NpcShopInstance) {
				...
				if (CharPosUtil.getZoneType(pc) == 1
						|| user.checkNonPvP(user, pc)
						|| CharPosUtil.getZoneType(user) == 1) {
					user.sendPackets(new S_UseAttackSkill(user, pc.getId(),
							11736, pc.getX(), pc.getY(), 17, 0), true);
					Broadcaster.broadcastPacket(user, new S_UseAttackSkill(
							user, pc.getId(), 11736, pc.getX(), pc.getY(), 17,
							0), true);
					return;
				}
			}
			user.sendPackets(
					new S_UseAttackSkill(user, npc.getId(), 11736, npc.getX(),
							npc.getY(), 17), true);
			Broadcaster.broadcastPacket(user,
					new S_UseAttackSkill(user, npc.getId(), 11736, npc.getX(),
							npc.getY(), 17), true);
